...
static int do_mkdir(const char *path, mode_t mode){
    ...
    if(print_info_command("MKDIR", path)){
        mkdir(fpath, 0777);
...
static int do_rename(const char *from, const char *to){
    ...
    char fpath_from[500], fpath_to[500];
...
static int do_readlink(const char *path, char *buf, size_t size){
    ...
    if(print_info_command("READLINK", path)){
...
static int do_chmod(const char *path, uid_t uid, gid_t gid){
    printf("enter => chmod");
